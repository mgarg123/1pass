...
*   **Password Breach Checking:** When enabled (it is on by default), the app checks if passwords have been compromised. To do this securely, it sends only a 5-character SHA-1 hash prefix of the password to the public HaveIBeenPwned API (k-anonymity). Neither your password nor the full hash ever leaves your device. This is an intentional, opt-out exception to the fully-local encryption model, and users who want maximum privacy should disable this feature in Settings.
*   **Time-Based One-Time Passwords (TOTP):** The generation of TOTP codes relies entirely on your device's local system clock. If your device's clock drifts significantly from true time, the generated 2FA codes will be invalid. This is not a security flaw, but rather a functional requirement of the RFC 6238 standard.
